...
Windows 删除文件或文件夹时经常只提示“文件已在另一程序中打开”，但不告诉你到底是谁占用。可以安装 [LockHunter](https://lockhunter.com/download.htm)：

1. 右键删不掉的文件或文件夹。
2. 选择 **What's locking this file?**。
3. 查看占用它的进程。
4. 确认不是系统关键进程后，再选择解锁、删除、重命名，或安排下次重启时删除。

!!! warning "先看清进程再操作"
    LockHunter 可以结束占用进程，误操作可能导致正在编辑的文件丢失或程序异常退出。遇到系统目录、驱动、数据库、虚拟机镜像等文件时，优先关闭相关软件或重启后再处理。

### 右键快速转换文件格式

[File Converter](https://file-converter.io/) 是一个开源 Windows 右键菜单转换工具，适合把图片、音频、视频、PDF 等文件快速转成常用格式。
...
安装后在资源管理器中选中文件，右键即可看到转换选项。Office 文档转换通常需要本机已安装并激活 Microsoft Office。
